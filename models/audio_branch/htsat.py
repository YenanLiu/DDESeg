...
from . import PatchEmbed, Mlp, DropPath, trunc_normal_, to_2tuple 
...
class HTSAT_Swin_Transformer(nn.Module):
    ...
    def __init__(self, spec_size=256, patch_size=4, patch_stride=(4,4), 
        ...
        if isinstance(patch_stride, str):
            patch_stride = eval(patch_stride)  
    ...
    def forward_features(self, x):
        frames_num = x.shape[2] # 256
        x = self.patch_embed(x) # torch.Size([2, 64*64, 96]) 
        if self.ape:
            x = x + self.absolute_pos_embed
        x = self.pos_drop(x)
        for i, layer in enumerate(self.layers):
            x, attn = layer(x) # torch.Size([2, 64, 768]) torch.Size([2, 32, 64, 64])
    
        if self.config["enable_tscam"]:
            # for x
            x = self.norm(x) # torch.Size([2, 64, 768])
            B, N, C = x.shape # 2, 64, 768
            SF = frames_num // (2 ** (len(self.depths) - 1)) // self.patch_stride[0] # 8
            ST = frames_num // (2 ** (len(self.depths) - 1)) // self.patch_stride[1] # 8
            x = x.permute(0,2,1).contiguous().reshape(B, C, SF, ST) # torch.Size([2, 768, 8, 8])
            B, C, F, T = x.shape # [2, 768, 8, 8]
            # group 2D CNN
            c_freq_bin = F // self.freq_ratio # 2
            x = x.reshape(B, C, F // c_freq_bin, c_freq_bin, T) # torch.Size([2, 768, 4, 2, 8])
            x = x.permute(0,1,3,2,4).contiguous().reshape(B, C, c_freq_bin, -1) # torch.Size([2, 768, 2, 32])

            # get latent_output
            latent_output = self.avgpool(torch.flatten(x,2)) # torch.Size([2, 768, 1])
            latent_output = torch.flatten(latent_output, 1) # torch.Size([2, 768])
            ...
            # torch.Size([2, 768, 2, 32])
            x = self.tscam_conv(x) # torch.Size([2, 71, 1, 32])
            x = torch.flatten(x, 2) # torch.Size([2, 71, 32]) 
        
            if self.config["htsat_attn_heatmap"]:
                fpx = interpolate(torch.sigmoid(x).permute(0,2,1).contiguous() * attn, 8 * self.patch_stride[1]) 
            else: 
                fpx = interpolate(torch.sigmoid(x).permute(0,2,1).contiguous(), 8 * self.patch_stride[1])  # torch.Size([2, 1024, 71])
        
            x = self.avgpool(x) # torch.Size([2, 71, 1])
            x = torch.flatten(x, 1) # torch.Size([2, 71])
            ...
            if self.config["loss_type"] == "clip_ce":
                ...
                output_dict = {
                    'framewise_output': fpx, # already sigmoided torch.Size([2, 1024, 71])
                    'clipwise_output': torch.sigmoid(x), # torch.Size([2, 71])
                    'latent_output': latent_output # torch.Size([2, 768])
                }
    ...
    def repeat_wat2img(self, x, cur_pos):
        ...
        return x
    # X: torch.Size([4, 22050])
    def forward(self, x: torch.Tensor, mixup_lambda = None, infer_mode = False):# out_feat_keys: List[str] = None):
        x = self.spectrogram_extractor(x)   # (batch_size, 1, time_steps, freq_bins) 
        x = self.logmel_extractor(x)    # (batch_size, 1, time_steps, mel_bins) torch.Size([4, 1, 87, 513])
        
        x = x.transpose(1, 3) # torch.Size([4, 64, 87, 1])
        x = self.bn0(x)
        x = x.transpose(1, 3)
        if self.training:
